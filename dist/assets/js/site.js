...
(function () {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          ...
          if (!d) return;
          if (e.isIntersecting) d.classList.add("is-active");
  ...
  function animateCounters() {
    ...
    els.forEach((el) => {
      const target = String(
        el.getAttribute("data-counter") || el.textContent || "0"
      ).replace(/[^\d.,]/g, "");
  ...
  function enableReels() {
    $$(".reel").forEach((reel) => {
      ...
      reel.addEventListener("pointerdown", (e) => {
        isDown = true;
        startX = e.clientX;
        scrollL = reel.scrollLeft;
        reel.setPointerCapture(e.pointerId);
      });
      reel.addEventListener("pointermove", (e) => {
        if (!isDown) return;
        reel.scrollLeft = scrollL - (e.clientX - startX);
      });
      ["pointerup", "pointercancel", "mouseleave"].forEach((ev) =>
        reel.addEventListener(ev, () => {
          isDown = false;
        })
      );
    });
  ...
  function markSSRReady() {
    $$('.section[aria-busy="true"]').forEach((sec) => {
      if (sec.textContent.trim().length > 0)
  ...
  function initHeaderSquarespace() {
    ...
    function openMega(id) {
      ...
      panelsWrap
        .querySelectorAll(".mega__section")
    ...
    if (primary && mega) {
      ...
      document.addEventListener("click", (e) => {
        if (mega && mega.dataset.state === "open" && !header.contains(e.target))
          closeMega();
      });
    }
    ...
    function applyTheme() {
      ...
      if (themeLabel)
        themeLabel.textContent = saved.charAt(0).toUpperCase() + saved.slice(1);
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Tab" && mega && mega.dataset.state === "open") {
        const focusable = mega.querySelectorAll(
    ...
    if (primary && panelsWrap) {
      ...
      liWithNoId.forEach((li, i) => {
        ...
        const id =
          (section ? section.getAttribute("data-panel") : null) || "auto-" + i;
