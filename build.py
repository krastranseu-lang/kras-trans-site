...
def similarity(a, b):
    sa, sb = set(a.get("tags", []) or []), set(b.get("tags", []) or [])
    j = len(sa & sb) / (len(sa | sb) or 1)
    # lekki bonus za wspólne słowa w tytule
    wa = words(a.get("title"))
    wb = words(b.get("title"))
    bonus = len(wa & wb) / (len(wa) or 1) * 0.2
    return j + bonus
...
def compute_related(pages):
    ...
    # wzajemność linków (delikatny boost interlinków)
    for p in pages:
        for r in list(p.get("related", [])):
            if p["slug"] not in by_slug.get(r, {}).get("related", []):
...
def jsonld_localbusiness(company):
    ...
    if c.get("logo"):
        lb["logo"] = SITE_URL + "/static/img/" + c["logo"]
    ...
    if c.get("opening_hours"):
        lb["openingHours"] = [c["opening_hours"]]
        ...
        "itemListElement": [
            {
                ...
                "position": i + 1,
                "name": p["title"],
                "item": SITE_URL + "/" + (p["slug"] + "/" if p["slug"] else "")
            }
...
def service_schema(page, company):
    ...
    prov = {"@type": "Organization", "name": (company[0].get("name", "Kras-Trans") if company else "Kras-Trans")}
    area = page.get("service_area") or (company[0].get("area_served") if company else None)
    return {
        "@context": "https://schema.org",
        "@type": "Service",
        "name": page.get("title"),
...
def main():
    ...
    for p in pages:
        if not p.get("publish", True):
            continue
        p["slug"] = (p.get("slug") or slugify(p.get("title", "page"))).strip("/")
    ...
    # --- skopiuj static (CSS/IMG) do artefaktu ---
    ...
    for p in pages:
        ...
        html = page_tpl.render(
            page=p, pages=pages, related=rel, company=company, head=head, site_url=SITE_URL
        )
    ...
    home = {
        "slug": "",
        "url": "/",
        "title": "Kras-Trans — Ekspresowy transport 24/7",
        "h1": "Ekspresowy transport 24/7 — Polska & UE",
        "lead": "Busy do 3,5 t i TIR. Rodzinna firma z Łodzi. Zadzwoń: +48 793 927 467",
        "hero_image": "placeholder-hero-desktop.webp",
        "hero_image_mobile": "placeholder-hero-desktop.webp",
        "html": ""
    }
    head_home = {
        "title": home["title"],
        "description": "Transport ekspresowy i dedykowany — Polska & Europa.",
        "canonical": SITE_URL + "/",
        "og_title": home["title"],
    ...
    ensure_file(
        OUT / "index.html",
        page_tpl.render(page=home, pages=pages, related=[], company=company, head=head_home, site_url=SITE_URL)
    )
    ...
    sm.append(f'<url><loc>{SITE_URL}/</loc><lastmod>{today}</lastmod></url>')
    for p in pages:
        sm.append(f'<url><loc>{SITE_URL}{p["url"]}</loc><lastmod>{today}</lastmod></url>')
    sm.append('</urlset>')
    ...
    # --- Redirects (HTML meta refresh – kompatybilne z GH Pages) ---
    for r in redirects:
